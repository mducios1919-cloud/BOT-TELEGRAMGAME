...
@app.post("/api/tick")
def api_tick():
    """One-shot endpoint cho PHP/website ngoài.
    Body JSON: { "service": "Views", "url": "https://..." }
    Trả về: { ok, session_id, service, wait_seconds, raw_head } hoặc { error }.
    Tạo session mới, tự OCR captcha, gửi 1 tick, giữ session lại cho lần sau (LRU tự dọn)."""
    data = request.json or {}
    service = data.get("service")
    url = data.get("url")
    if not service or not url:
    ...
    client: ZefoyClient = s["client"]
    try:
        r0 = client.session.get(client.base_url, timeout=30)
        if is_captcha_page(r0.text):
            apply_session_guard_cookies(client.session)
            cap = client.get_captcha(refresh_session=False)
            s["captcha_bytes"] = cap.image_bytes
            s["captcha_encoded"] = build_captcha_encoded(client.user_agent)
            text = ""
            try:
                text = (NewOcrWeb().ocr(cap.image_bytes).text or "")
            except Exception:
                text = ""
            if not re.sub(r'[^a-zA-Z]', '', text):
                try:
                    from zefoy.ocr import solve_ddddocr
                    text = solve_ddddocr(cap.image_bytes) or ""
                except Exception:
                    pass
            text = re.sub(r'[^a-zA-Z]', '', text or '').lower()
            if not text:
                return jsonify({"error": "ocr empty", "session_id": sid}), 200
            # submit answer inline
            encoded = s.get("captcha_encoded") or build_captcha_encoded(client.user_agent)
            apply_session_guard_cookies(client.session)
            rs = client.session.post(
                client.base_url,
                headers={
                    "user-agent": client.user_agent,
                    "content-type": "application/x-www-form-urlencoded; charset=UTF-8",
                ...
                data={"captchalogin": text, "captcha_encoded": encoded},
                timeout=30, allow_redirects=False,
            )
            body = (rs.text or "").strip()
            ok_login = rs.status_code == 200 and body.lower() == "success"
            if not ok_login and not is_captcha_page(body) and len(body) > 1000:
                ok_login = True
            if not ok_login:
                return jsonify({"error": "captcha_failed", "session_id": sid, "answer": text}), 200
            s["logged_in"] = True
            _refresh_services(s)
        else:
            s["logged_in"] = True
        ...
        return jsonify({"error": str(e), "session_id": sid}), 500
    low = body.lower()
    if "session expired" in low or is_captcha_page(body):
        return jsonify({"error": "session_expired", "session_id": sid}), 200
    m = re.search(r'(\d+)\s*seconds?\s*(?:for your next|before trying)', body, re.I)
    wait = int(m.group(1)) if m else None
    return jsonify({"ok": True, "session_id": sid, "service": service,
                    "wait_seconds": wait, "raw_head": body[:300]})
